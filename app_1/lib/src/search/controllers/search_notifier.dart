...
class SearchNotifier with ChangeNotifier {
  ...
  void searchFunction(String searchKey) async {
    ...
    // Mã hóa từ khóa tìm kiếm để đảm bảo các ký tự đặc biệt như tiếng Việt được truyền chính xác
    String encodedSearchKey = Uri.encodeComponent(searchKey);

    /*print(
        "Searching for: $searchKey (Encoded: $encodedSearchKey)"); */ // Debugging bước tìm kiếm bắt đầu

    Uri url = Uri.parse(
        '${Environment.appBaseUrl}/api/products/search/?q=$encodedSearchKey');
